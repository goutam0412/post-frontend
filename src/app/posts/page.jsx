...
import { Trash2, Calendar, Plus, Eye, Edit3, TrendingUp } from "lucide-react";
...
export default function PostsContent() {
  ...
  useEffect(() => {
    fetchPosts();
  }, [page, perPage, postFilter]);

  const fetchPosts = async () => {
    try {
      setLoading(true);

      let urlString = `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/posts?`;

      if (postFilter.status !== "All") {
        urlString += `status=${postFilter.status}&`;
      }
